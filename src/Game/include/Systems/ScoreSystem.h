...
class ScoreSystem : public Engine::System {
   public:
    void OnScoreIncreased(const ScoreIncreasedEvent& event,
                          BreakoutECS& system) {
    ...
    void OnRender(BreakoutECS& system) {
        for (const auto entity : m_Entities) {
            ...
            text.text.SetString(
                std::to_string(score.score));  // Set the correct score.
            text.text.CenterOrigin();          // Center the origin.
            text.text.SetPosition(
                transform.location);  // Set the origin position.
    ...
    static void RegisterSelf(BreakoutECS& system) {
        ...
        system.AddListner<ScoreIncreasedEvent>(
            [weakSelf, &system](const ScoreIncreasedEvent& event) {
