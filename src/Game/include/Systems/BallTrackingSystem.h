       ...
       public:
        void OnUpdate(BreakoutECS& system) {
                if (m_CheckBallAmount) {
                        if (m_Entities.empty()) system.SendEvent(LifeUsedEvent{});

                        m_CheckBallAmount = false;
                }
        }
        void OnBallDestruction() { m_CheckBallAmount = true; }
        ...
        static void RegisterSelf(BreakoutECS& system) {
                ...
                system.AddListner<BallDestroyedEvent>([weakSelf](const BallDestroyedEvent&) {
