       ...
       public:
        void OnUpdate(BreakoutECS& system) {
                if (m_CheckBallAmount) {
                        if (m_Entities.empty()) {
                                // Do Box Spawning.
                                // The position should have some lerp, to move them into the scene.

                                // For now i will just spawn a single box.
                                RegisterBox(system, Engine::Renderer::Get().GetViewSize() / 2.0f);
        ...
        static void RegisterSelf(BreakoutECS& system) {
                ...
                system.AddListner<BoxDestroyedEvent>([weakSelf](const BoxDestroyedEvent&) {
