...
void Breakout::BallLifeSystem::LifeGained(BreakoutECS& system) {
    ...
    if (availableLifeIncrease) {
        // Update the entity.
        BallSlots& slot =
            system.GetComponent<BallSlots>(first_available_entity);
        slot.empty = false;

        Sprite& sprite = system.GetComponent<Sprite>(first_available_entity);
        sprite.sprite =
            Engine::ManagedSprite("assets/Textures/AvailableBall.png");
    }
}
...
void Breakout::BallLifeSystem::LifeUsed(BreakoutECS& system) {
    ...
    bool AvailableLifeDecrease =
        false;  // Should never be false, as the game would be over otherwise.
