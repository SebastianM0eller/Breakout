...
void ComponentManager<EntityCount>::RegisterComponent() {
    ...
    if constexpr (!std::is_empty_v<T>) {
        m_ComponentArrays.insert(
            {typeName, std::make_shared<ComponentArray<T, EntityCount>>()});
    }
...
T& ComponentManager<EntityCount>::GetComponent(Entity entity) {
    static_assert(!std::is_empty_v<T>,
                  "Can't retrieve data for an empty struct");
    return GetComponentArray<T>()->GetData(entity);
}

///
/// Removes the entity's components from the respective ComponentArrays, while
/// keeping them dense.
///
template <uint32_t EntityCount>
...
template <uint32_t EntityCount>
template <typename T>
std::shared_ptr<ComponentArray<T, EntityCount>>
ComponentManager<EntityCount>::GetComponentArray() {
    std::type_index typeName(typeid(T));

    assert(m_ComponentTypes.find(typeName) != m_ComponentTypes.end() &&
           "Component not registered before use.");

    return std::static_pointer_cast<ComponentArray<T, EntityCount>>(
