...
class ECS {
   public:
    ...
    void Init() {
        ...
        m_EntityManager =
            std::make_unique<EntityManager<ComponentCount, EntityCount>>();
    ...
    /// An assert is triggered in debug, if the component has not yet been
    /// registered. An assert is also triggered if the entity already has an
    ...
    /// An assert is triggered in debug, if the entity does not have the
    /// component, or if the component is not registered.
    ///
    template <typename T>
    void RemoveComponent(Entity entity) {
        ...
        Signature signature = m_EntityManager->GetSignature(entity);
        signature.set(m_ComponentManager->template GetComponentType<T>(),
                      false);
        m_EntityManager->SetSignature(entity, signature);
