...
class EventManager {
       ...
       private:
        std::unordered_map<std::type_index, std::list<std::shared_ptr<void>>> m_EventListners;
};
...
void EventManager::AddListner(std::function<void(const EventType&)> callback) {
        ...
        auto wrapper = std::make_shared<std::function<void(const EventType&)>>(callback);
...
void EventManager::SendEvent(const EventType& event) {
        ...
        if (m_EventListners.find(typeName) == m_EventListners.end()) return;

        for (auto& genericCallback : m_EventListners[typeName]) {
                // Exstract the method
                auto callback = std::static_pointer_cast<std::function<void(const EventType&)>>(genericCallback);
