...
///
/// Removes the component of the entity, from the ComponentArray while keeping
/// the ComponentArray dense. Asserts that the entity has the component trying
/// to be removed.
///
template <typename T, uint32_t EntityCount>
void ComponentArray<T, EntityCount>::RemoveData(Entity entity) {
    ...
    m_ComponentArray[indexOfRemovedEntity] =
        m_ComponentArray[indexOfLastElement];  // Does it need to be deleted?
...
///
/// Removes the component associated with the destroyed entity, while keeping
/// the ComponentArray dense. If the entity has no associated component, this
/// does nothing.
///
template <typename T, uint32_t EntityCount>
