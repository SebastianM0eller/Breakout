...
/// Finds a value at a specific percentage (blend) between the start and the
/// target. Uses standard linear interpolation: start + (target - start) * blend
///
inline float Lerp(float start, float target, float blend) {
