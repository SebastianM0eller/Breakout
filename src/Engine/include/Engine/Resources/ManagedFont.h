...
class ManagedFont {
   public:
    ManagedFont() : m_Font(nullptr) {
    }  // We need default initialization, for it to work with the ECS.

    // Used to load the actual font.
    ManagedFont(const std::string& path)
        : m_Font(ResourceManager::Get().Load<sf::Font>(path)) {
    }
    ...
    ~ManagedFont() {
        if (m_Font != nullptr)
            ResourceManager::Get().Remove(m_Font);
    }
